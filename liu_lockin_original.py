#!/usr/bin/env python3
"""
Liu lock-in photon counting (quadrature square-wave RWC).

Implements the reference-weighted counting (RWC) scheme with two ±1 square-wave
references I and Q shifted by 90°, as described by Liu et al.:
...
- Outputs (per bin): I, Q (signed), and S = |I| + |Q| (photon-number output).

Paper context (citations refer to your attached PDF):
- Quadrature demodulation with I/Q square waves, 90° phase shift, ±1 weights:
  “a pair of parallel accumulators … with the in-phase and quadrature reference
   signals … mathematically regarded as bipolar square-wave of ‘±1’ weights …”
- RWC corresponds to multiply (+1/–1) at each event and integrate over Tint:
  “operation of adding ‘1’ or subtracting ‘1’ … for multiple periods (integration)”.

CLI
---
python liu_lockin.py <file.ptu>
  ...
  python liu_lockin_original.py data/august18/Amplitude-Phase-Flatness/500Hz-1mW-1s.ptu --N 10 --duty 0.5 
...
# Import your updated reader for PicoHarp 330 T2 (CH0 markers, CH1 photons)
...
def local_phase_fractions(
    ...
    Robust to small wander in f_mod; requires len(ref_edges) >= N.
    """
    ...
    # rolling pointer to the most recent edge <= t
    j = N - 1
    n_idx = np.arange(N, dtype=np.float64)
    n_mean = n_idx.mean()

    for i, t in enumerate(photon_times_ps):
        # advance j so r[j] <= t < r[j+1]
        while j + 1 < r.size and r[j + 1] <= t:
            j += 1
        j_use = max(j, N - 1)
        j0 = j_use - (N - 1)
        window = r[j0 : j_use + 1]  # length N

        # linear regression: n = a * time + b  -> period T = 1/a (edges per ps inverted)
        t_mean = window.mean()
        t_center = window - t_mean
        denom = float((t_center**2).sum())
        if denom == 0.0:
            # fallback to last delta if all equal (shouldn't happen in real data)
            T = float(window[-1] - window[-2])
            if T <= 0:
                T = 1.0
        else:
            a = float(((t_center) * (n_idx - n_mean)).sum()) / denom  # edges per ps
            if a == 0:
                T = float(window[-1] - window[-2])
                if T <= 0:
                    T = 1.0
            else:
                T = 1.0 / a  # ps per edge -> the period
...
def sq_weights(phi: np.ndarray, duty: float = 0.5, phase_shift: float = 0.0) -> np.ndarray:
    """
    Bipolar square-wave weights in {+1, -1} at phases phi∈[0,1).

    +1 during ON window, -1 during OFF. An ideal 50% duty puts the ON window
    on [phase_shift, phase_shift + 0.5) modulo 1.
    """
...
def crop_by_time(
    ...
    """Return boolean mask selecting times within [start_s, stop_s)."""
...
def make_bins(start_ps: int, stop_ps: int, bin_sec: float) -> np.ndarray:
    """Edges (ps) for uniform time bins over [start_ps, stop_ps]."""
    if bin_sec <= 0:
...
def estimate_global_freq(ref_edges_ps: np.ndarray) -> float:
    """Rough global f_mod [Hz] from median period between consecutive ref edges."""
...
def run(
    ...
    # per bin compute phases, demod, print
    if csv:
    ...
    for b in range(edges.size - 1):
        ...
        # select photons/ref edges in this bin
        ...
        # If too few edges in this bin, expand to use neighboring edges for phase (still only count photons in bin)
        if rf_bin.size < N:
            # grab neighbor edges around the bin limits
            # indices in refs for nearest edges to [b0,b1)
